...
const Dashboard: React.FC<DashboardProps> = ({ products, sales }) => {
  ...
  const metrics = useMemo(() => {
    ...
    const monthlyProfit = sales
      ...
      .reduce((sum, s) => {
        const product = products.find(p => p.id === s.itemCode);
        const buyingPrice = product?.buyingPrice || s.unitPrice * 0.7; 
        return sum + (s.totalAmount - (buyingPrice * s.quantity));
      }, 0);

    const lowStockCount = products.filter(p => p.stock <= 15).length;

    return { todaySales, monthlySales, monthlyProfit, lowStockCount };
  }, [sales, products]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-[#1d70d1] p-8 rounded-[2.5rem] text-white shadow-xl shadow-blue-100 relative overflow-hidden h-44 flex flex-col justify-between transition-transform hover:scale-[1.02]">
           <div>
             <p className="text-blue-100 text-[10px] font-bold uppercase tracking-widest opacity-80">Today Sales (LKR)</p>
             <h3 className="text-2xl md:text-3xl font-black mt-1">{metrics.todaySales.toLocaleString()} LKR</h3>
           </div>
        ...
        <div className="bg-[#64748b] p-8 rounded-[2.5rem] text-white shadow-xl shadow-slate-100 relative overflow-hidden h-44 flex flex-col justify-between transition-transform hover:scale-[1.02]">
           <div>
             <p className="text-slate-200 text-[10px] font-bold uppercase tracking-widest opacity-80">Total Products</p>
             <h3 className="text-2xl md:text-3xl font-black mt-1">{products.length} items</h3>
           </div>
           <div className="absolute bottom-8 right-8 w-12 h-12 bg-[#ef4444] rounded-2xl flex items-center justify-center font-black text-sm shadow-lg shadow-red-200 animate-pulse">
             {metrics.lowStockCount}
           </div>
        </div>

        {/* Card 3: Monthly Profit (Green) */}
        <div className="bg-[#10b981] p-8 rounded-[2.5rem] text-white shadow-xl shadow-emerald-100 relative overflow-hidden h-44 flex flex-col justify-between transition-transform hover:scale-[1.02]">
           <div>
             <p className="text-emerald-50 text-[10px] font-bold uppercase tracking-widest opacity-80">Monthly Profit</p>
             <h3 className="text-2xl md:text-3xl font-black mt-1">{metrics.monthlyProfit.toLocaleString()} LKR</h3>
           </div>
           <div className="flex items-center justify-end">
              <div className="bg-white/20 p-2 rounded-xl">
                <TrendingUp className="w-6 h-6 text-white" />
